...
class SinusoidalPositionalEmbedding(nn.Module):
    def __init__(self, block_size, embedding_dim):
        self.table = torch.Tensor([[torch.sin(pos / 10000 ** (2 * i / embedding_dim)) if (i % 2 == 0) else torch.cos(pos / 10000 ** (2 * i / embedding_dim)) for i in range(embedding_dim)] for pos in range(block_size)])
    def forward(self, x):
        _, T = x.shape
